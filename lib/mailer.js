...
function getPublicSiteUrl() {
  ...
  return 'https://animalconcept.com';
}
        ...
        <tr>
          <td style="background:#111113;border-radius:12px 12px 0 0;padding:32px 40px;border-bottom:2px solid #C9762E;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  <div style="font-size:20px;font-weight:900;color:#ffffff;letter-spacing:0.04em;">ANIMAL CONCEPT SRL</div>
                  <div style="font-size:9px;letter-spacing:0.4em;color:#C9762E;text-transform:uppercase;margin-top:3px;">Élevage familial · Oupeye</div>
                </td>
                <td align="right">
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="font-size:12px;color:rgba(255,255,255,0.3);line-height:1.8;">
                  <strong style="color:rgba(255,255,255,0.5);">ANIMAL CONCEPT SRL</strong><br/>
...
async function sendReservationConfirmation({ email, name, reservation, puppy }) {
  ...
  const content = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Profession</div>
          <div style="font-size:14px;color:#fff;">${reservation.guestProfession || '—'}</div>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Adresse domicile</div>
          <div style="font-size:14px;color:#fff;">${reservation.guestHomeAddress || '—'}</div>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Téléphone</div>
          <div style="font-size:14px;color:#fff;">${reservation.guestPhone || '—'}</div>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Animal à la maison</div>
          <div style="font-size:14px;color:#fff;">${formatBool(reservation.hasPet)}</div>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Déjà perdu un animal</div>
          <div style="font-size:14px;color:#fff;">${formatBool(reservation.hasLostPet)}</div>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Livraison</div>
          <div style="font-size:14px;color:#fff;">${reservation.deliveryMethod === 'delivery' ? '🚚 Livraison' : '🏠 Retrait sur place'}</div>
        </td>
      </tr>
      ${reservation.deliveryMethod === 'delivery' ? `<tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Adresse livraison</div>
          <div style="font-size:14px;color:#fff;">${reservation.deliveryAddress || '—'}</div>
        </td>
      </tr>` : ''}
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Prix du chiot</div>
          <div style="font-size:14px;color:#fff;">${formatEuro(puppy.price)}</div>
        </td>
      </tr>
      ${reservation.discountPercent > 0 ? `<tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Réduction (-${reservation.discountPercent}%)</div>
          <div style="font-size:14px;color:#22c55e;">– ${formatEuro(reservation.discountAmount)}</div>
        </td>
      </tr>` : ''}
      <tr>
        <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Acompte versé</div>
      ...
      ${reservation.balanceAmount > 0 ? `<tr>
        <td style="padding:14px 0;">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Solde à payer (à la remise)</div>
          <div style="font-size:14px;color:#fff;">${formatEuro(reservation.balanceAmount)}</div>
        </td>
      </tr>` : ''}
      ${reservation.notes ? `<tr>
        <td style="padding:14px 0;">
          <div style="font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:4px;">Message</div>
          <div style="font-size:14px;color:#fff;white-space:pre-wrap;">${reservation.notes}</div>
        </td>
      </tr>` : ''}
    </table>
...
async function sendAdminNotification({ reservation, puppy }) {
  ...
  const content = `
    <div style="text-align:center;margin-bottom:28px;">
      ...
      <h1 style="margin:0 0 6px;font-size:24px;font-weight:900;color:#ffffff;">Nouvelle réservation</h1>
      <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.4);">Une demande vient d'arriver</p>
    </div>
    <div style="background:rgba(201,118,46,0.06);border:1px solid rgba(201,118,46,0.2);border-radius:10px;padding:18px 22px;text-align:center;margin-bottom:24px;">
      <div style="font-size:11px;font-weight:800;letter-spacing:0.35em;text-transform:uppercase;color:rgba(255,255,255,0.3);margin-bottom:6px;">N° Réservation</div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Client</span><br/><span style="color:#fff;font-size:15px;font-weight:700;">${reservation.guestName}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Email</span><br/><span style="color:#fff;font-size:15px;">${reservation.guestEmail}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Téléphone</span><br/><span style="color:#fff;font-size:15px;">${reservation.guestPhone}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Profession</span><br/><span style="color:#fff;font-size:14px;">${reservation.guestProfession || '—'}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Adresse domicile</span><br/><span style="color:#fff;font-size:14px;">${reservation.guestHomeAddress || '—'}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Animal à la maison</span><br/><span style="color:#fff;font-size:14px;">${formatBool(reservation.hasPet)}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Déjà perdu un animal</span><br/><span style="color:#fff;font-size:14px;">${formatBool(reservation.hasLostPet)}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Livraison</span><br/><span style="color:#fff;font-size:14px;">${reservation.deliveryMethod === 'delivery' ? '🚚 Livraison' : '🏠 Retrait sur place'}${reservation.deliveryMethod === 'delivery' && reservation.deliveryAddress ? ` (${reservation.deliveryAddress})` : ''}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Chiot</span><br/><span style="color:#fff;font-size:15px;font-weight:700;">${puppy.name} (${puppy.breed})</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Prix</span><br/><span style="color:#C9762E;font-size:16px;font-weight:800;">${formatEuro(puppy.price)}</span></td></tr>
      <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Paiement</span><br/><span style="color:#fff;font-size:15px;">${reservation.paymentLabel || (reservation.paymentMethod === 'full' ? 'Intégral' : 'Acompte 50%')}</span></td></tr>
      ${reservation.notes ? `<tr><td style="padding:10px 0;"><span style="color:rgba(255,255,255,0.35);font-size:12px;">Notes client</span><br/><span style="color:#fff;font-size:14px;">${reservation.notes}</span></td></tr>` : ''}
    </table>
...
async function sendReplyToCustomer({ email, name, subject, message }) {
  const content = `
    <div style="text-align:center;margin-bottom:28px;">
      ...
      <h1 style="margin:0 0 6px;font-size:24px;font-weight:900;color:#ffffff;">Message de ANIMAL CONCEPT SRL</h1>
      <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.4);">Bonjour ${name},</p>
    </div>
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:24px;margin-bottom:24px;">
      <p style="font-size:15px;color:#e0e0e0;line-height:1.75;margin:0;white-space:pre-wrap;">${message}</p>
    </div>
    <p style="font-size:13px;color:rgba(255,255,255,0.25);text-align:center;margin:0;">
...
module.exports = { sendReservationConfirmation, sendAdminNotification, sendReplyToCustomer };
