...
async function sendReservationConfirmation({ email, name, reservation, puppy }) {
  ...
  await sendMail({
    ...
    text: [
      ...
      `Montant : ${formatEuro(reservation.depositAmount || 0)}${reservation.balanceAmount > 0 ? ` + ${formatEuro(reservation.balanceAmount)}` : ''}`,
...
async function sendReplyToCustomer({ email, name, subject, message }) {
  const content = `
    <div style="text-align:center;margin-bottom:28px;">
      ...
      <h1 style="margin:0 0 6px;font-size:24px;font-weight:900;color:#ffffff;">Message de Sweet Puppies</h1>
  ...
  await sendMail({
    to: email,
    subject: subject || 'Sweet Puppies — Message de votre éleveur',
    html: baseTemplate(content),
    text: [
      `Message de Sweet Puppies`,
      ``,
      `Bonjour ${name},`,
      ``,
      message,
